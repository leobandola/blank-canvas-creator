...
export function PrizesRoundsList({ rounds }: { rounds: Round[] }) {
  if (rounds.length === 0) {
    return (<Card><div className="py-12 text-center"><p className="text-muted-foreground">Nenhuma rodada cadastrada ainda.</p></div></Card>);
  }
        ...
        <Card key={round.id} className="hover:shadow-lg transition-shadow">
          <CardHeader>
            <div className="flex items-start justify-between">
              ...
              <Button asChild className="bg-purple-600 hover:bg-purple-700"><Link to={`/prizes/${round.id}`}><Trophy className="h-4 w-4 mr-2" />Ver Classificação</Link></Button>
            </div>
          </CardHeader>
        </Card>
      ))}
