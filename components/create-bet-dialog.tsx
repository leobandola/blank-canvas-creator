    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <div className="flex flex-wrap gap-1 p-3 border rounded-md min-h-[60px] bg-muted/50">
                {selectedNumbers.length === 0 ? (
                  <span className="text-sm text-muted-foreground">Selecione os números abaixo</span>
                ) : (
                  selectedNumbers
                    .sort((a, b) => a - b)
                    .map((num) => (
                ...
                {Array.from({ length: maxNumber }, (_, i) => i + 1).map((num) => (
                  <Button
                    ...
                    className={`h-9 w-full text-xs font-mono ${
                      selectedNumbers.includes(num) ? "bg-amber-600 hover:bg-amber-700" : ""
                    }`}
          ...
          <DialogFooter>
            <Button type="submit" disabled={isLoading || selectedNumbers.length !== maxSelections || !selectedPlayer}>
