...
export function EditDrawDialog({ 
  draw, 
  lotteryType 
}: { 
  draw: Draw, 
  ...
  useEffect(() => {
    if (open) {
      setSelectedNumbers(draw.numbers);
      setDrawDate(new Date(draw.draw_date).toISOString().split('T')[0]);
    }
  }, [open, draw]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedNumbers.length !== requiredNumbers) {
      alert(`Você deve selecionar exatamente ${requiredNumbers} números.`);
      return;
    }
    ...
    if (error) {
      ...
      alert("Erro ao atualizar sorteio: " + error.message);
    } else {
      setOpen(false);
      router.refresh();
    }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle>Editar Sorteio #{draw.draw_number}</DialogTitle>
            <DialogDescription>
              Atualize a data e os números do sorteio. Escolha {requiredNumbers} números de 1 a {maxNumber}.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              <div className="flex items-center justify-between">
                <Label>
                  Números Sorteados: {selectedNumbers.length}/{requiredNumbers}
                </Label>
          ...
          <DialogFooter>
            <Button
              type="submit"
              disabled={isLoading || selectedNumbers.length !== requiredNumbers}
            >
